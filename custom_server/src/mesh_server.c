...
void server_send_to_client(model_sensor_data_t server_model_state)
{
    ...
    // ESP_LOGI(TAG, "Sending data of size: %d", server_model_state.device_name);
    //
    // esp_err_t  err = esp_ble_mesh_model_publish(custom_models,ESP_BLE_MESH_CUSTOM_SENSOR_MODEL_OP_STATUS,
    // sizeof(server_model_state), (uint8_t *)&server_model_state,ROLE_NODE);
    if(err != ESP_OK){
