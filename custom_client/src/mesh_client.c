...
static void ble_mesh_custom_sensor_client_model_cb(esp_ble_mesh_model_cb_event_t event,
                                                   esp_ble_mesh_model_cb_param_t *param) {
    switch (event) {
        case ESP_BLE_MESH_MODEL_OPERATION_EVT: // nhận bản tin từ server
        ...
        case ESP_BLE_MESH_CLIENT_MODEL_RECV_PUBLISH_MSG_EVT: // nhận bản tin status từ server
            switch (param->client_recv_publish_msg.opcode) {
                case ESP_BLE_MESH_CUSTOM_SENSOR_MODEL_OP_STATUS:
                    ...
                    if (strcmp(received_data.device_name, "esp_server 01") == 0) {
                        mqtt_data_publish_callback(json_data,"node-01");
                    }
                    else if(strcmp(received_data.device_name, "esp_server 02") == 0)
                    {
                        mqtt_data_publish_callback(json_data,"node-02");
                    }
                    free(json_data);
                break;
...
esp_err_t ble_mesh_custom_sensor_client_model_message_get(void) {
    ...
    // err = esp_ble_mesh_model_publish(custom_sensor_client.model, opcode, 0, NULL, ROLE_NODE);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to send the custom msg 0x%06x, err = 0x%06x", opcode, err);
    }
